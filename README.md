...
- **Listing.** S2 lists streams in lexicographic order, so each stream is named by an inverted timestamp; the basin is dedicated to RePlaya, so `streams.list()` over its root returns newest-first with `startAfter` paging — no database keeping an order in sync. A best-effort sidecar index stream is tailed to update the list as new sessions start.
